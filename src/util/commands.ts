...
import {
    randomNumberWithCurveInRange,
    randomNumberWithCurve,
    randomNumberWithExpDist,
    randomNumberWithExpDistInRange,
} from "./math";
import {
    Argument,
    GlobalCommands,
    CommandRunArgs,
} from "../types/interface/commands.js";
...
const globalCommands: GlobalCommands = {
    help: {
        description:
            "Displays a list of available commands, or details about a given command",
        arguments: [
            {
                alias: ["c", "command"],
                required: false,
        ...
        run: function (args: CommandRunArgs): string {
            if (args.tutorial) {
                ...
                output +=
                    "The console is a command line interface that allows you to interact with the game.\n";
                output +=
                    "To use the console, simply type a command and press enter. For example, to run the help command, type 'help' and press enter.\n";
                output +=
                ...
                output +=
                    "Arguments can have many different types, which are boolean, number, and string.\n";
                output +=
                ...
                output +=
                    "Strings and numbers require a little more than just specifying the flag. They need to be followed up by a value.\n";
                output +=
                ...
                output +=
                    "That concludes the tutorial. If you have additional questions, you're fucked. Good luck!";
                return output;
            }
            ...
            if (!args.command) {
                ...
                var output =
                    "Run 'help --command [command]' to get more information about a specific command\n";
                output +=
                    "Run 'help --tutorial' for a tutorial on how to use the console\n\n";
                output += "The following commands are available:\n";
                for (var i = 0; i < commands.length; i++) {
                    console.log(commands[i] + " a");
                    output += `  ${commands[i]}: ${
                        globalCommands[commands[i]].description
                    }\n`;
                }
                return output;
            } else {
                ...
                if (globalCommands[command]) {
                    return `Displaying info for command '${command}':\nDescription: ${
                        globalCommands[command].description
                    }${commandHandler.renderArgs(
    ...
    ping: {
        ...
        run: async function (args: CommandRunArgs): Promise<string> {
            ...
            return (await CommandHandler.delayExecutionThenReturn(
                result * 2,
                `Ping: ${result.toFixed(4)}`
            )) as string;
        },
    },
    ...
    throttle: {
        description:
            "Makes engine go brrr (may take a second to spool up or down)",
        arguments: [
            {
                alias: ["f", "full"],
                required: false,
        ...
        run: async function (args: CommandRunArgs): Promise<string> {
            ...
            if (args.full) {
                return (await CommandHandler.delayExecutionThenReturn(
                    delay,
                    "FULLL THROTTLER BSABYYYYY"
                )) as string;
            }
            if (args.stop) {
                return (await CommandHandler.delayExecutionThenReturn(
                    delay,
                    "what a pussy. the engine is stopped, but out of spite."
                )) as string;
            }
            if (args.set) {
                return (await CommandHandler.delayExecutionThenReturn(
                    delay,
                    `Throttle set to ${args.set}%.`
                )) as string;
            }
            return (await CommandHandler.delayExecutionThenReturn(
                0,
                "IDFK what the throttle is set to LOL"
            )) as string;
        },
    },
};
